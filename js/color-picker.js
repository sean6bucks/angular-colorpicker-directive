 ...
 */
'use strict';
var colorPicker = angular.module('sk-colorpicker', [])
...
colorPicker.directive('colorPicker', ['$document', '$compile', 'ColorHelper', function ($document, $compile, ColorHelper) {
    return {
        restrict: 'EA',
        scope: { colorPickerModel: '=', colorPickerAlphaModel: '=', colorPickerOptions: '=', colorPickerOutputFormat: '=' },
        controller: [ '$scope', function ($scope) {
            ...
            $scope.alphaClass = '';
            $scope.options = $scope.colorPickerOptions || {};

            if ( $scope.options.defaultType !== undefined ) {
                $scope.type = $scope.options.defaultType;
            } else {
                var outputFormat = $scope.options.outputFormat || $scope.colorPickerOutputFormat;
            ...
            $scope.update = function () {
                ...
                $scope.hexText = { 
                    hex: '#' + ((1 << 24) | (parseInt(rgba.r, 10) << 16) | (parseInt(rgba.g, 10) << 8) | parseInt(rgba.b, 10)).toString(16).substr(1),
                    a: Math.round(rgba.a * 100)
                };

                if ($scope.hexText.hex[1] === $scope.hexText.hex[2] && $scope.hexText.hex[3] === $scope.hexText.hex[4] && $scope.hexText.hex[5] === $scope.hexText.hex[6]) {
                    $scope.hexText.hex = '#' + $scope.hexText.hex[1] + $scope.hexText.hex[3] + $scope.hexText.hex[5];
                }
                ...
                if ($scope.hsva.a < 1) {
                    ...
                    switch (outputFormat) {
                        ...
                        case 'rgba':
                            ...
                            break;
                        default:
                            $scope.outputColor = $scope.hexText.hex;
                            break;
                    }
                }

                $scope.sAndLSlider = {left: $scope.hsva.s * $scope.sAndLMax.x - 8 + 'px', top: (1 - $scope.hsva.v) * $scope.sAndLMax.y - 8 + 'px'};
                $scope.hueSlider.left = ($scope.hsva.h) * $scope.hueMax.x - 4 + 'px';
                $scope.alphaSlider.left = $scope.hsva.a * $scope.alphaMax.x - 4 + 'px';
        ...
        link: function (scope, element, attr) {
            ...
            // SET DEFAULTS FROM ATTR OR OPTIONS OBJECT
            if ( scope.colorPickerModel === undefined ) {
                scope.colorPickerModel = scope.options.defaultColor || '#008fff';
                element.val('');
            }

            if (attr.colorPickerShowValue === undefined) {
                attr.colorPickerShowValue = scope.options.showValue || 'true';
            }
            if (attr.colorPickerPosition === undefined) {
                attr.colorPickerPosition = scope.options.position || 'right';
            }
            if (attr.colorPickerShowInputSpinner === undefined) {
                attr.colorPickerShowInputSpinner = scope.options.showInputSpinner || 'false';
            }
            
            if (attr.colorPickerShowCancelButton === undefined) {
                attr.colorPickerShowCancelButton = scope.options.showCancelButton || 'false';
            }
            ...
            if (attr.colorPickerShowAlpha === undefined) {
                attr.colorPickerShowAlpha = scope.options.showAlpha || 'true';
            }
            if (attr.colorPickerShowAlpha === 'false') {
                scope.showAlpha = false;
                scope.alphaClass = 'no-alpha';
                // var alphaBg = document.createElement('div');
                // alphaBg.className = "color-picker-alpha-background"
                // element.append( alphaBg );
            }
            
            // CHECK OPTIONS FOR ANY OPTIONAL VALUES
            if (attr.colorPickerCancelButtonClass === undefined) {
                attr.colorPickerCancelButtonClass = scope.options.cancelButtonClass || undefined;
            }
            if (attr.colorPickerCancelButtonClass !== undefined) {
                scope.cancelButtonClass = attr.colorPickerCancelButtonClass;
            }

            if (attr.colorPickerFixedPosition === undefined) {
                attr.colorPickerFixedPosition = scope.options.fixedPosition || undefined;
            }

            if (attr.colorPickerSpinnerRgbaSteps === undefined) {
                attr.colorPickerSpinnerRgbaSteps = scope.options.spinnerRgbaSteps || undefined;
            }
            ...
            if (attr.colorPickerSpinnerHslaSteps === undefined) {
                attr.colorPickerSpinnerHslaSteps = scope.options.spinnerHslaSteps || undefined;
            }
            ...
            template = angular.element(
                '<div ng-show="show" class="color-picker {{extraLargeClass}} {{ alphaClass }}">' +
                ...
                '   </div>' +
                '   <div slider rg-x=1 action="setAlpha(v, rg)" class="alpha" ng-show="showAlpha" ng-style="{\'background-color\':alphaSliderColor}">' +
                '       <div class="cursor" ng-style="{\'left\':alphaSlider.left}"></div>' +
                '   </div>' +
                '   <div class="selected-color-background"></div>' +
                '   <div class="selected-color" ng-style="{\'background-color\':outputColor, \'opacity\':outputAlpha}"></div>' +
                ...
                '       <input text type="number" pattern="[0-9]+([\.,][0-9]{1,2})?" min="0" max="1" step="' + scope.hslaSteps.a + '" rg=1 action="setAlpha(v, rg)" ng-show="showAlpha" ng-model="hslaText.a" spinner="' + attr.colorPickerShowInputSpinner + '" />' +
                '       <div>H</div><div>S</div><div>L</div><div ng-show="showAlpha">A</div>' +
                '   </div>' +
                ...
                '       <input text type="number" pattern="[0-9]+([\.,][0-9]{1,2})?" min="0" max="1" step="' + scope.rbgaSteps.a + '" rg=1 action="setAlpha(v, rg)" ng-model="rgbaText.a" spinner="' + attr.colorPickerShowInputSpinner + '" ng-show="showAlpha"2 />' +
                '       <div>R</div><div>G</div><div>B</div><div ng-show="showAlpha">A</div>' +
                '   </div>' +
                '   <div class="hex-text" ng-show="type==0">' +
                '       <input class="hex-input" text type="text" action="setColorFromHex(string)" ng-model="hexText.hex"/>' +
                '       <input class="alpha-input" text type="number" pattern="[0-9]{1,2,3}" min="0" max="100" step="1" rg=100 ng-model="hexText.a" action="setAlpha(v, rg)" ng-show="showAlpha"/>' +
                '       <div>HEX</div>' +
                '       <div ng-show="showAlpha">%</div>' +
                '   </div>' +
            ...
            scope.$on('color-changed', function (event) {
                scope.$apply(function () {
                    ...
                    scope.colorPickerAlphaModel = scope.outputAlpha;
                    if (attr.colorPickerShowValue === 'true') {
                        element.val(scope.outputColor);
                    }
                });
            ...
            function setDialogPosition() {
                ...
                var dialog = { width: 232, height: 262 };
                if ( scope.extraLargeClass == 'color-picker-extra-large' ) dialog.height = 292;

                if (attr.colorPickerPosition === "left") {
                    template[0].style.top = box.top + 'px';
                    template[0].style.left = (box.left - dialog.width - 10 ) + 'px';
                } else if (attr.colorPickerPosition === "top") {
                    template[0].style.top = (box.top - dialog.height - 10) + 'px';
